...
在调用 API 时，可以通过以下方式操作 "我的文档库"：

1. **获取子节点列表**：调用[获取知识空间子节点列表](https://go.feishu.cn/s/65W4PEw0I04)接口时，将 `space_id` 指定为 `my_library`，即可获取 "我的文档库" 中的文档列表

2. **获取文档内容**：对于 "我的文档库" 中的文档，可通过以下步骤获取内容：

   

   - 先获取节点的 `obj_token`（文档的唯一标识）
   - 再调用对应文档类型的内容获取接口（如文档调用[获取文档内容](https://go.feishu.cn/s/671HilYwo03)接口）

3. **权限控制**：应用访问 "我的文档库" 需获得用户授权，可通过以下方式：

   

   - 将应用添加为文档协作者
   - 使用 user_access_token 访问用户自有资源

如何导出知识库中文档？
通过调用 获取节点信息 接口，可以从返回值中获取到 obj_type 和 obj_token。
